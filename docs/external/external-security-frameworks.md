...
Veil **does not ship** full JCSF/DAF spreadsheets in-repo — those directories are **read-only reference** at cxado meta root [`refs/`](../../../../refs/) (see [cxado-references](https://github.com/butbeautifulv/cxado-references)). Operational truth lives in:
...
4. **`refs/` at cxado meta root is never executed** — same rule as HexStrike reference; no CI dependency on PDF binaries. `.external/Anthropic-Cybersecurity-Skills-main/` remains Veil-local for `make corpus-import` only.
